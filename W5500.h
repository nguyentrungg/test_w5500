 ...
 * @ref REG_MR is used for S/W reset, ping block mode, PPPoE mode and etc.
 * @details Each bit of @ref REG_MR defined as follows.
 * <table>
 ...
 */
#define REG_MR                 (_W5500_IO_BASE_ + (0x0000 << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief Gateway IP Register address(R/W)
 * @details @ref REG_GAR configures the default gateway address.
 */
#define REG_GAR                (_W5500_IO_BASE_ + (0x0001 << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief Subnet mask Register address(R/W)
 * @details @ref REG_SUBR configures the subnet mask address.
 */
#define REG_SUBR               (_W5500_IO_BASE_ + (0x0005 << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief Source MAC Register address(R/W)
 * @details @ref REG_SHAR configures the source hardware address.
 */
#define REG_SHAR               (_W5500_IO_BASE_ + (0x0009 << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief Source IP Register address(R/W)
 * @details @ref REG_SIPR configures the source IP address.
 */
#define REG_SIPR               (_W5500_IO_BASE_ + (0x000F << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief Set Interrupt low level timer register address(R/W)
 * @details @ref REG_INTLEVEL configures the Interrupt Assert Time.
 */
#define REG_INTLEVEL           (_W5500_IO_BASE_ + (0x0013 << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief Interrupt Register(R/W)
 * @details @ref REG_IR indicates the interrupt status. Each bit of @ref REG_IR will be still until the bit will be written to by the host.
 * If @ref REG_IR is not equal to x00 INTn PIN is asserted to low until it is x00\n\n
 * Each bit of @ref REG_IR defined as follows.
 * <table>
 *                 <tr>  <td>7</td> <td>6</td> <td>5</td> <td>4</td> <td>3</td> <td>2</td> <td>1</td> <td>0</td>   </tr>
 ...
 */
#define REG_IR                 (_W5500_IO_BASE_ + (0x0015 << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief Interrupt mask register(R/W)
 * @details @ref REG_IMR is used to mask interrupts. Each bit of @ref REG_IMR corresponds to each bit of @ref REG_IR.
 * When a bit of @ref REG_IMR is and the corresponding bit of @ref REG_IR is  an interrupt will be issued. In other words,
 * if a bit of @ref REG_IMR is  an interrupt will not be issued even if the corresponding bit of @ref REG_IR is \n\n
 * Each bit of @ref REG_IMR defined as the following.
 * <table>
 *                 <tr>  <td>7</td> <td>6</td> <td>5</td> <td>4</td> <td>3</td> <td>2</td> <td>1</td> <td>0</td>   </tr>
 ...
 */
#define REG_IMR                (_W5500_IO_BASE_ + (0x0016 << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief Socket Interrupt Register(R/W)
 * @details @ref REG_SIR indicates the interrupt status of Socket.\n
 * Each bit of @ref REG_SIR be still until @ref Sn_IR is cleared by the host.\n
 * If @ref Sn_IR is not equal to x00 the n-th bit of @ref REG_SIR is and INTn PIN is asserted until @ref REG_SIR is x00 */
#define REG_SIR                (_W5500_IO_BASE_ + (0x0017 << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief Socket Interrupt Mask Register(R/W)
 * @details Each bit of @ref REG_SIMR corresponds to each bit of @ref REG_SIR.
 * When a bit of @ref REG_SIMR is and the corresponding bit of @ref REG_SIR is  Interrupt will be issued.
 * In other words, if a bit of @ref REG_SIMR is  an interrupt will be not issued even if the corresponding bit of @ref REG_SIR is
 */
#define REG_SIMR               (_W5500_IO_BASE_ + (0x0018 << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief Timeout register address( 1 is 100us )(R/W)
 * @details @ref REG_RTR configures the retransmission timeout period. The unit of timeout period is 100us and the default of @ref REG_RTR is x07D0or 000
 * And so the default timeout period is 200ms(100us X 2000). During the time configured by @ref REG_RTR, W5500 waits for the peer response
 * to the packet that is transmitted by \ref Sn_CR (CONNECT, DISCON, CLOSE, SEND, SEND_MAC, SEND_KEEP command).
 * If the peer does not respond within the @ref REG_RTR time, W5500 retransmits the packet or issues timeout.
 */
#define REG_RTR                (_W5500_IO_BASE_ + (0x0019 << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief Retry count register(R/W)
 * @details @ref REG_RCR configures the number of time of retransmission.
 * When retransmission occurs as many as ref REG_RCR+1 Timeout interrupt is issued (@ref Sn_IR[TIMEOUT] = .
 */
#define REG_RCR                (_W5500_IO_BASE_ + (0x001B << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief PPP LCP Request Timer register  in PPPoE mode(R/W)
 * @details @ref REG_PTIMER configures the time for sending LCP echo request. The unit of time is 25ms.
 */
#define REG_PTIMER             (_W5500_IO_BASE_ + (0x001C << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief PPP LCP Magic number register  in PPPoE mode(R/W)
 * @details @ref REG_PMAGIC configures the 4bytes magic number to be used in LCP negotiation.
 */
#define REG_PMAGIC             (_W5500_IO_BASE_ + (0x001D << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief PPP Destination MAC Register address(R/W)
 * @details @ref REG_PHAR configures the PPPoE server hardware address that is acquired during PPPoE connection process.
 */
#define REG_PHAR                (_W5500_IO_BASE_ + (0x001E << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief PPP Session Identification Register(R/W)
 * @details @ref REG_PSID configures the PPPoE sever session ID acquired during PPPoE connection process.
 */
#define REG_PSID               (_W5500_IO_BASE_ + (0x0024 << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief PPP Maximum Segment Size(MSS) register(R/W)
 * @details @ref REG_PMRU configures the maximum receive unit of PPPoE.
 */
#define REG_PMRU               (_W5500_IO_BASE_ + (0x0026 << 8) + (WIZCHIP_CREG_BLOCK << 3))
 ...
 * which socket is not open and @ref UNREACH bit of @ref REG_IR becomes and @ref REG_UIPR & @ref REG_UPORTR indicates
 * the destination IP address & port number respectively.
 */
#define REG_UIPR               (_W5500_IO_BASE_ + (0x0028 << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief Unreachable Port register address in UDP mode(R)
 * @details W5500 receives an ICMP packet(Destination port unreachable) when data is sent to a port number
 * which socket is not open and @ref UNREACH bit of @ref REG_IR becomes and @ref REG_UIPR & @ref REG_UPORTR
 * indicates the destination IP address & port number respectively.
 */
#define REG_UPORTR              (_W5500_IO_BASE_ + (0x002C << 8) + (WIZCHIP_CREG_BLOCK << 3))

/**
 * @ingroup Common_register_group
 * @brief PHY Status Register(R/W)
 * @details @ref REG_PHYCFGR configures PHY operation mode and resets PHY. In addition, @ref REG_PHYCFGR indicates the status of PHY such as duplex, Speed, Link.
 */
#define REG_PHYCFGR            (_W5500_IO_BASE_ + (0x002E << 8) + (WIZCHIP_CREG_BLOCK << 3))
 ...
 * @details @ref REG_VERSIONR always indicates the W5500 version as @b 0x04.
 */
#define REG_VERSIONR           (_W5500_IO_BASE_ + (0x0039 << 8) + (WIZCHIP_CREG_BLOCK << 3))
 ...
 * the corresponding bit of @ref Sn_IR becomes  When both the corresponding bit of @ref Sn_IMR and @ref Sn_IR are and the n-th bit of @ref REG_IR is
 ...
 * When this bit is  Destination Information such as IP address and Port number may be checked with the corresponding @ref REG_UIPR & @ref REG_UPORTR.
 */
 ...
 * When this bit is  It sends the ACK packet after waiting for the timeout time configured by @ref REG_RTR.
 */
 ...
 */
#define setGAR(gar) \
                WIZCHIP_WRITE_BUF(REG_GAR,gar,4)
 ...
 */
#define getGAR(gar) \
                WIZCHIP_READ_BUF(REG_GAR,gar,4)
 ...
 */
#define setSUBR(subr) \
                WIZCHIP_WRITE_BUF(REG_SUBR, subr,4)
 ...
 */
#define getSUBR(subr) \
                WIZCHIP_READ_BUF(REG_SUBR, subr, 4)
 ...
 */
#define setSHAR(shar) \
                WIZCHIP_WRITE_BUF(REG_SHAR, shar, 6)
 ...
 */
#define getSHAR(shar) \
                WIZCHIP_READ_BUF(REG_SHAR, shar, 6)
 ...
 */
#define setSIPR(sipr) \
                WIZCHIP_WRITE_BUF(REG_SIPR, sipr, 4)
 ...
 */
#define getSIPR(sipr) \
                WIZCHIP_READ_BUF(REG_SIPR, sipr, 4)

/**
 * @ingroup Common_register_access_function
 * @brief Set REG_INTLEVEL register
 * @param (uint16_t)intlevel Value to set @ref REG_INTLEVEL register.
 * @sa getINTLEVEL()
 */
#define setINTLEVEL(intlevel)  {\
                WIZCHIP_WRITE(REG_INTLEVEL,   (uint8_t)(intlevel >> 8)); \
                WIZCHIP_WRITE(WIZCHIP_OFFSET_INC(REG_INTLEVEL,1), (uint8_t) intlevel); \
        }


/**
 * @ingroup Common_register_access_function
 * @brief Get REG_INTLEVEL register
 * @return uint16_t. Value of @ref REG_INTLEVEL register.
 * @sa setINTLEVEL()
 */
#define getINTLEVEL() \
                ((WIZCHIP_READ(REG_INTLEVEL) << 8) + WIZCHIP_READ(WIZCHIP_OFFSET_INC(REG_INTLEVEL,1)))

/**
 * @ingroup Common_register_access_function
 * @brief Set @ref REG_IR register
 * @param (uint8_t)ir Value to set @ref REG_IR register.
 * @sa getIR()
 */
#define setIR(ir) \
                WIZCHIP_WRITE(REG_IR, (ir & 0xF0))

/**
 * @ingroup Common_register_access_function
 * @brief Get @ref REG_IR register
 * @return uint8_t. Value of @ref REG_IR register.
 * @sa setIR()
 */
#define getIR() \
                (WIZCHIP_READ(REG_IR) & 0xF0)
/**
 * @ingroup Common_register_access_function
 * @brief Set @ref REG_IMR register
 * @param (uint8_t)imr Value to set @ref REG_IMR register.
 * @sa getIMR()
 */
#define setIMR(imr) \
                WIZCHIP_WRITE(REG_IMR, imr)

/**
 * @ingroup Common_register_access_function
 * @brief Get @ref REG_IMR register
 * @return uint8_t. Value of @ref REG_IMR register.
 * @sa setIMR()
 */
#define getIMR() \
                WIZCHIP_READ(REG_IMR)


/**
 * @ingroup Common_register_access_function
 * @brief Set @ref REG_SIR register
 * @param (uint8_t)sir Value to set @ref REG_SIR register.
 * @sa getSIR()
 */
#define setSIR(sir) \
                WIZCHIP_WRITE(REG_SIR, sir)

/**
 * @ingroup Common_register_access_function
 * @brief Get @ref REG_SIR register
 * @return uint8_t. Value of @ref REG_SIR register.
 * @sa setSIR()
 */
#define getSIR() \
                WIZCHIP_READ(REG_SIR)
/**
 * @ingroup Common_register_access_function
 * @brief Set @ref REG_SIMR register
 * @param (uint8_t)simr Value to set @ref REG_SIMR register.
 * @sa getSIMR()
 */
#define setSIMR(simr) \
                WIZCHIP_WRITE(REG_SIMR, simr)

/**
 * @ingroup Common_register_access_function
 * @brief Get @ref REG_SIMR register
 * @return uint8_t. Value of @ref REG_SIMR register.
 * @sa setSIMR()
 */
#define getSIMR() \
                WIZCHIP_READ(REG_SIMR)

/**
 * @ingroup Common_register_access_function
 * @brief Set @ref REG_RTR register
 * @param (uint16_t)rtr Value to set @ref REG_RTR register.
 * @sa getRTR()
 */
#define setRTR(rtr)   {\
                WIZCHIP_WRITE(REG_RTR,   (uint8_t)(rtr >> 8)); \
                WIZCHIP_WRITE(WIZCHIP_OFFSET_INC(REG_RTR,1), (uint8_t) rtr); \
        }

/**
 * @ingroup Common_register_access_function
 * @brief Get @ref REG_RTR register
 * @return uint16_t. Value of @ref REG_RTR register.
 * @sa setRTR()
 */
#define getRTR() \
                ((WIZCHIP_READ(REG_RTR) << 8) + WIZCHIP_READ(WIZCHIP_OFFSET_INC(REG_RTR,1)))

/**
 * @ingroup Common_register_access_function
 * @brief Set @ref REG_RCR register
 * @param (uint8_t)rcr Value to set @ref REG_RCR register.
 * @sa getRCR()
 */
#define setRCR(rcr) \
                WIZCHIP_WRITE(REG_RCR, rcr)

/**
 * @ingroup Common_register_access_function
 * @brief Get @ref REG_RCR register
 * @return uint8_t. Value of @ref REG_RCR register.
 * @sa setRCR()
 */
#define getRCR() \
                WIZCHIP_READ(REG_RCR)

//================================================== test done ===========================================================

/**
 * @ingroup Common_register_access_function
 * @brief Set @ref REG_PTIMER register
 * @param (uint8_t)ptimer Value to set @ref REG_PTIMER register.
 * @sa getPTIMER()
 */
#define setPTIMER(ptimer) \
                WIZCHIP_WRITE(REG_PTIMER, ptimer)

/**
 * @ingroup Common_register_access_function
 * @brief Get @ref REG_PTIMER register
 * @return uint8_t. Value of @ref REG_PTIMER register.
 * @sa setPTIMER()
 */
#define getPTIMER() \
                WIZCHIP_READ(REG_PTIMER)

/**
 * @ingroup Common_register_access_function
 * @brief Set @ref REG_PMAGIC register
 * @param (uint8_t)pmagic Value to set @ref REG_PMAGIC register.
 * @sa getPMAGIC()
 */
#define setPMAGIC(pmagic) \
                WIZCHIP_WRITE(REG_PMAGIC, pmagic)

/**
 * @ingroup Common_register_access_function
 * @brief Get @ref REG_PMAGIC register
 * @return uint8_t. Value of @ref REG_PMAGIC register.
 * @sa setPMAGIC()
 */
#define getPMAGIC() \
                WIZCHIP_READ(REG_PMAGIC)

/**
 * @ingroup Common_register_access_function
 * @brief Set REG_PHAR address
 * @param (uint8_t*)phar Pointer variable to set PPP destination MAC register address. It should be allocated 6 bytes.
 * @sa getPHAR()
 */
#define setPHAR(phar) \
                WIZCHIP_WRITE_BUF(REG_PHAR, phar, 6)
 ...
 */
#define getPHAR(phar) \
                WIZCHIP_READ_BUF(REG_PHAR, phar, 6)

/**
 * @ingroup Common_register_access_function
 * @brief Set @ref REG_PSID register
 * @param (uint16_t)psid Value to set @ref REG_PSID register.
 * @sa getPSID()
 */
#define setPSID(psid)  {\
                WIZCHIP_WRITE(REG_PSID,   (uint8_t)(psid >> 8)); \
                WIZCHIP_WRITE(WIZCHIP_OFFSET_INC(REG_PSID,1), (uint8_t) psid); \
        }

/**
 * @ingroup Common_register_access_function
 * @brief Get @ref REG_PSID register
 * @return uint16_t. Value of @ref REG_PSID register.
 * @sa setPSID()
 */
//uint16_t getPSID(void);
#define getPSID() \
                ((WIZCHIP_READ(REG_PSID) << 8) + WIZCHIP_READ(WIZCHIP_OFFSET_INC(REG_PSID,1)))

/**
 * @ingroup Common_register_access_function
 * @brief Set @ref REG_PMRU register
 * @param (uint16_t)pmru Value to set @ref REG_PMRU register.
 * @sa getPMRU()
 */
#define setPMRU(pmru) { \
                WIZCHIP_WRITE(REG_PMRU,   (uint8_t)(pmru>>8)); \
                WIZCHIP_WRITE(WIZCHIP_OFFSET_INC(REG_PMRU,1), (uint8_t) pmru); \
        }

/**
 * @ingroup Common_register_access_function
 * @brief Get @ref REG_PMRU register
 * @return uint16_t. Value of @ref REG_PMRU register.
 * @sa setPMRU()
 */
#define getPMRU() \
                ((WIZCHIP_READ(REG_PMRU) << 8) + WIZCHIP_READ(WIZCHIP_OFFSET_INC(REG_PMRU,1)))
 ...
 */
#define getUIPR(uipr) \
                WIZCHIP_READ_BUF(REG_UIPR,uipr,6)

/**
 * @ingroup Common_register_access_function
 * @brief Get @ref REG_UPORTR register
 * @return uint16_t. Value of @ref REG_UPORTR register.
 */
#define getUPORTR() \
        ((WIZCHIP_READ(REG_UPORTR) << 8) + WIZCHIP_READ(WIZCHIP_OFFSET_INC(REG_UPORTR,1)))

/**
 * @ingroup Common_register_access_function
 * @brief Set @ref REG_PHYCFGR register
 * @param (uint8_t)phycfgr Value to set @ref REG_PHYCFGR register.
 * @sa getPHYCFGR()
 */
#define setPHYCFGR(phycfgr) \
                WIZCHIP_WRITE(REG_PHYCFGR, phycfgr)

/**
 * @ingroup Common_register_access_function
 * @brief Get @ref REG_PHYCFGR register
 * @return uint8_t. Value of @ref REG_PHYCFGR register.
 * @sa setPHYCFGR()
 */
#define getPHYCFGR() \
                WIZCHIP_READ(REG_PHYCFGR)

/**
 * @ingroup Common_register_access_function
 * @brief Get @ref REG_VERSIONR register
 * @return uint8_t. Value of @ref REG_VERSIONR register.
 */
#define getVERSIONR() \
                WIZCHIP_READ(REG_VERSIONR)
